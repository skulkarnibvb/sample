<script type='text/javascript'>
	...
	function taskFormValidator() {
		...
		var myInputs = new Array();
					myInputs[i] = document.getElementById("incD");
					i++;


		var j=0;
						if(notEmpty(myInputs[j]) && !isAlphanumeric(myInputs[j])) {
							alert("Please enter valid incD");
							myInputs[j].focus();
							return false;
						}
		...
		<fieldset>
          ...
          <fieldset>
            <legend>Task Outputs</legend>
                            		<label for="name">incD</label>
                            		<div class="div_texbox">
                              		<input name="incD" type="text" class="textbox" id="incD" value="${incD}" />
                            		</div>
